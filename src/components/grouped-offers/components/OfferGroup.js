...
function OfferGroup({
  ...
  const { addToBasket } = useBasket(selection?.data?.attributes); // This is the addToBasket function from the context
    ...
    <div className={`offer-group-container ${bestValue ? 'best-value' : ''}`}>
      ...
      <div className='offers-container'>
        <div className='offers-description'>
          {thumbnail ? <img src={thumbnail.placeholder} alt='' /> : null}
        ...
        <div className='offer-selection'>
          ...
          >
            <Button
              ...
              onClick={() =>
                addToBasket(selection, redirect_url + window.location.search)
              }
            >
              {ctaText || 'Buy now'}
            </Button>
      ...
      <div className='additional-offers-link'>
        <a
          onClick={() => (window.location = buttonUrl + window.location.search)}
        >
          {buttonText}
        </a>
      </div>
